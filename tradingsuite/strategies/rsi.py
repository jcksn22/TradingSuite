...
def rsi_strategy(data, buy_threshold = 30, sell_threshold = 70):
    ...
    for i in range(1, len(data)):
        ...
        if not in_trade:
            # Generate buy signal
            #You have to change only the buy and sell signal
...
def show_indicator_rsi_strategy(ticker, buy_threshold = 30, sell_threshold = 70, plot_title = '', ndays=0, plot_height=1000, add_strategy_summary = True, use_tradingview_title = False):
    """
    ...
    if use_tradingview_title and plot_title == '':
        try:
            ...
            plot_title = f'{ticker} - RSI Strategy'
    elif plot_title == '':
        plot_title = f'{ticker} - RSI Strategy'
...
# show_indicator_rsi_strategy('AAPL', 30, 70, use_tradingview_title=True)
